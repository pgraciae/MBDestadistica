...
onehotTest <- data.frame(predict(onehot, newdata = p2_test)) 
onehotTest
```
...
#### 5.2 Predicción test
```{r}
predFinal <- predict(mod_rf, newdata = select(onehotTest,-y))

finalDF <- data.frame(id = p2_test_fin$id, gt = y_pred) #se obtiene el id del dataset original
finalDF
```
